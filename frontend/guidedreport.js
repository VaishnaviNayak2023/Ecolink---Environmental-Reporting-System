/* ============================================================
   Updated guidedreport.js (Dummy Submission)
   - Leaflet map (Nominatim search)
   - Category selection enforced
   - Dynamic review population + small review-map
   - File preview
   - Dummy success message for submission
   ============================================================ */
...
(() => {
  ...
  document.getElementById('submitReport').addEventListener('click', () => {
    ...
    setTimeout(() => {
      const dummyReportId = "GR-DUMMY123";
      const dummyFiles = Array.from(fileUpload.files || []).map(f => f.name);
      let msg = `<div class="success-box">✅ Report submitted successfully!`;
      msg += `<div><strong>Report ID:</strong> ${dummyReportId}</div>`;
      if (dummyFiles.length) {
        msg += `<div><strong>Uploaded Files:</strong><ul>`;
        dummyFiles.forEach(f => msg += `<li>${f}</li>`);
        msg += `</ul></div>`;
      }
      msg += `</div>`;
